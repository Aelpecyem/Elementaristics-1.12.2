...
public class ChunkGeneratorMind implements IChunkGenerator {
    ...
    protected static final IBlockState FABRIC_PASSION = ModBlocks.fabric_passion.getDefaultState();
    protected static final IBlockState BEDROCK = Blocks.BEDROCK.getDefaultState();
    protected static final IBlockState FABRIC_REASON = ModBlocks.fabric_reason.getDefaultState();
   private final World world;
    private final boolean generateStructures;
    private final Random rand;
    /** Holds the noise used to determine whether slowsand can be generated at a location */
    private double[] noise1 = new double[256];
    private double[] noise2 = new double[256];
    ...
    private NoiseGeneratorOctaves noise12;
    /** Determines whether something other than nettherack can be generated at a location */
    private NoiseGeneratorOctaves noiseExclusive;
    ...
    public ChunkGeneratorMind(World worldIn, boolean genStructures, long seed)
    {
        this.world = worldIn;
        this.generateStructures = genStructures;
        this.rand = new Random(seed);
        this.lperlinNoise1 = new NoiseGeneratorOctaves(this.rand, 2);//16, both
        this.lperlinNoise2 = new NoiseGeneratorOctaves(this.rand, 8);
        this.perlinNoise1 = new NoiseGeneratorOctaves(this.rand, 8);
        this.noise12 = new NoiseGeneratorOctaves(this.rand, 4);
        this.noiseExclusive = new NoiseGeneratorOctaves(this.rand, 2); //2
        this.scaleNoise = new NoiseGeneratorOctaves(this.rand, 40); //10
        this.depthNoise = new NoiseGeneratorOctaves(this.rand, 4); //16
        worldIn.setSeaLevel(63);

        net.minecraftforge.event.terraingen.InitNoiseGensEvent.ContextHell ctx =
                new net.minecraftforge.event.terraingen.InitNoiseGensEvent.ContextHell(lperlinNoise1, lperlinNoise2, perlinNoise1, noise12, noiseExclusive, scaleNoise, depthNoise);
        ctx = net.minecraftforge.event.terraingen.TerrainGen.getModdedNoiseGenerators(worldIn, this.rand, ctx);
        this.lperlinNoise1 = ctx.getLPerlin1();
        this.lperlinNoise2 = ctx.getLPerlin2();
        this.perlinNoise1 = ctx.getPerlin();
        this.noise12 = ctx.getPerlin2();
        this.noiseExclusive = ctx.getPerlin3();
                    ...
                    for (int i2 = 0; i2 < 8; ++i2)
                    {
                        ...
                        for (int j2 = 0; j2 < 4; ++j2)
                        {
                            ...
                            for (int k2 = 0; k2 < 4; ++k2)
                            {
                                ...
                                if (l1 * 8 + i2 < j)
                                {
                                    iblockstate = FABRIC_REASON;
                                }

                                if (d15 > 0.0D)
                                {
                                    iblockstate = FABRIC_PASSION;
                                }
    ...
    public void buildSurfaces(int p_185937_1_, int p_185937_2_, ChunkPrimer primer)
    {
        ...
        double d0 = 0.03125D;
        this.noise1 = this.noise12.generateNoiseOctaves(this.noise1, p_185937_1_ * 16, p_185937_2_ * 16, 0, 16, 16, 1, 0.03125D, 0.03125D, 1.0D);
        this.noise2 = this.noise12.generateNoiseOctaves(this.noise2, p_185937_1_ * 16, 109, p_185937_2_ * 16, 16, 1, 16, 0.03125D, 1.0D, 0.03125D);
        this.depthBuffer = this.noiseExclusive.generateNoiseOctaves(this.depthBuffer, p_185937_1_ * 16, p_185937_2_ * 16, 0, 16, 16, 1, 0.0625D, 0.0625D, 0.0625D);

        for (int z = 0; z < 16; ++z)
        {
            for (int x = 0; x < 16; ++x)
            {
                boolean flag = this.noise1[z + x * 16] + this.rand.nextDouble() * 0.2D > 0.0D;
                boolean flag1 = this.noise2[z + x * 16] + this.rand.nextDouble() * 0.2D > 0.0D;
                int l = (int)(this.depthBuffer[z + x * 16] / 3.0D + 3.0D + this.rand.nextDouble() * 0.25D);
                int i1 = -1;
                IBlockState iblockstate = FABRIC_PASSION;
                IBlockState iblockstate1 = FABRIC_REASON;
                ...
                for (int y = 127; y >= 0; --y)
                {
                    if (y < 127 - this.rand.nextInt(5) && y > this.rand.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == ModBlocks.fabric_passion)
                            {
                                if (i1 == -1)
                                {
                                    if (l <= 0)
                                    {
                                        iblockstate = AIR;
                                        iblockstate1 = FABRIC_PASSION;
                                    }
                                    else if (y >= i - 4 && y <= i + 1)
                                    {
                                        iblockstate = FABRIC_PASSION;
                                        iblockstate1 = FABRIC_REASON;
                                    ...
                                    if (y >= i - 1) //y > 80 = passion, else reason
                                    {
    ...
    public Chunk generateChunk(int x, int z)
    {
     ...
     /*   Biome abiome = BiomeInit.MIND;//getBiomeProvider().getBiomes((Biome[])null, x * 16, z * 16, 16, 16);
        byte[] abyte = chunk.getBiomeArray();

/*        abyte[0]= (byte)Biome.getIdForBiome(abiome);
        System.out.println(chunk.getBiomeArray());
