...
public class MindTerrainGenerator {
    ...
    public MindTerrainGenerator() {
        this.heightMap = new double[825];

        this.biomeWeights = new float[25];
        for (int j = -2; j <= 2; ++j) {
            for (int k = -2; k <= 2; ++k) {
                float f = 10.0F / (float) Math.sqrt((j * j + k * k) + 0.2F);
                        ...
                        for (int x = 0; x < 4; ++x) {
                            ...
                            for (int z = 0; z < 4; ++z) {
                                if (height < 2) {
                                    ...
                                    primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, ModBlocks.getElementEssenceFromName("mind").setBlockUnbreakable().getDefaultState());
                                }
                            }
