...
public class BlockForge extends BlockTileEntity<TileEntityForge> {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        TileEntityForge tile = getTileEntity(worldIn, pos);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            ...
            if (heldItem.getItem() instanceof ItemHammerHeat) {
                return false;
            }
            if (heldItem.isEmpty()) {
                ...
                if (tile.inventory.getStackInSlot(0).isEmpty()) {
                        ...
                        tile.markDirty();
                }else if (ForgeRecipes.getRecipeForInput(tile.inventory.getStackInSlot(0)) != null && tile.hitCount == ForgeRecipes.getRecipeForInput(tile.inventory.getStackInSlot(0)).hitAmount){
