...
public class RenderDimensionalNexus extends Render<EntityDimensionalNexus> {
    ...
    public void doRender(EntityDimensionalNexus entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!entity.isInvisible()) {
            ...
            float width = random.nextFloat() * 0.5F + varyLength;//random.nextFloat() * 2.0F + 1.0F + varyLength * 2.0F;
            ...
            for (int i = 0; i < amount; ++i) //(float)i < (f + f * f) / 2.0F * 60.0F //might vary the amount
            {
                ...
                bufferbuilder.pos(-0.866D * (double) width, (double) distance, (double) (-0.5F * width)).color(color.getRed(), color.getGreen(), color.getBlue(), 0).endVertex();
                bufferbuilder.pos(0.866D * (double) width, (double) distance, (double) (-0.5F * width)).color(color.getRed(), color.getGreen(), color.getBlue(), 0).endVertex();
                bufferbuilder.pos(0.0D, (double) distance, (double) (1.0F * width)).color(color.getRed(), color.getGreen(), color.getBlue(), 0).endVertex();
                bufferbuilder.pos(-0.866D * (double) width, (double) distance, (double) (-0.5F * width)).color(color.getRed(), color.getGreen(), color.getBlue(), 0).endVertex();
                tessellator.draw();
            }
