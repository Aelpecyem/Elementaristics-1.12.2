...
public class EntitySilverThread extends EntityMob {
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (cause.getTrueSource() instanceof EntityPlayer) {
            ...
            player = world.getClosestPlayer(posX, posY, posZ, 100, false);
  ...
  /*  @Override
    ...
    public boolean hitByEntity(Entity entity) {
        if (entity.hasCapability(PlayerCapProvider.ELEMENTARISTICS_CAP, null)) {
            IPlayerCapabilities cap = entity.getCapability(PlayerCapProvider.ELEMENTARISTICS_CAP, null);
            if (cap.getSoulId() == SoulInit.soulInstable.getId()) {
                world.playSound(posX, posY, posZ, SoundEvents.ENTITY_GUARDIAN_DEATH, SoundCategory.HOSTILE, 0.5F, 0.001F, true);
                this.damageEntity(DamageSource.OUT_OF_WORLD, 666);
                return true;
            }
        }
